...
impl MetadataWriter {
    ...
    pub async fn write_initial_metadata(
        ...
        // Write game metadata
        self.write_game_metadata(game_exe, game_config, game_resolution).await?;
    ...
    pub async fn finalize_session_metadata(
        ...
        let contents = fs::read_to_string(&path).await.map_err(|e| {
            eyre!("Failed to read session metadata: {}", e)
        })?;
        ...
        fs::write(&path, json).await.map_err(|e| {
            eyre!("Failed to write finalized session metadata: {}", e)
        })?;
    ...
    async fn write_hardware_metadata(&self) -> Result<()> {
        ...
        let metadata = HardwareMetadata {
            ...
            ram_gb: specs.ram_gb,
            os: format!("{:?}", specs.os),
            recording_drive: "NVMe SSD".to_string(), // TODO: Detect actual drive
        ...
        let json = serde_json::to_string_pretty(&metadata)?;
        fs::write(&path, json).await.map_err(|e| {
            eyre!("Failed to write hardware metadata: {}", e)
        })?;
    ...
    async fn write_game_metadata(
        ...
        let metadata = GameMetadata {
            ...
            version: game_config.version.clone().unwrap_or_else(|| "unknown".to_string()),
        ...
        let json = serde_json::to_string_pretty(&metadata)?;
        fs::write(&path, json).await.map_err(|e| {
            eyre!("Failed to write game metadata: {}", e)
        })?;
    ...
    async fn write_recorder_metadata(&self, settings: &EncoderSettings) -> Result<()> {
        ...
        let json = serde_json::to_string_pretty(&metadata)?;
        fs::write(&path, json).await.map_err(|e| {
            eyre!("Failed to write recorder metadata: {}", e)
        })?;
    ...
    pub async fn write_video_metadata(&self, metadata: &VideoMetadata) -> Result<()> {
        ...
        let json = serde_json::to_string_pretty(metadata)?;
        fs::write(&path, json).await.map_err(|e| {
            eyre!("Failed to write video metadata: {}", e)
        })?;
    ...
    pub async fn generate_checksums(&self) -> Result<()> {
        // Checksum recordings
        let recordings_checksums = self.checksum_directory(
            &self.session_manager.recordings_dir()
        ).await?;
        ...
        // Checksum streams
        let streams_checksums = self.checksum_directory(
            &self.session_manager.streams_dir()
        ).await?;
        self.write_checksum_file(
    ...
    async fn checksum_directory(&self, dir: &std::path::Path) -> Result<Vec<ChecksumEntry>> {
        ...
        while let Some(entry) = read_dir.next_entry().await? {
            ...
            if path.is_file() {
                ...
                let relative_path = path.strip_prefix(&self.session_manager.session_path())
...
mod tests {
    ...
    #[tokio::test]
    async fn test_metadata_writer() {
        let temp_dir = TempDir::new().unwrap();
        let session_manager = Arc::new(
            SessionManager::create(temp_dir.path(), "TestGame").await.unwrap()
        );
        
        let writer = MetadataWriter::new(session_manager.clone());
        
        // Create test config
        let game_config = GameConfig::default();
        let encoder_settings = EncoderSettings::default();
        ...
        // Check files exist
        assert!(session_manager.hardware_metadata_path().exists());
        assert!(session_manager.game_metadata_path().exists());
        assert!(session_manager.recorder_metadata_path().exists());
    }
}