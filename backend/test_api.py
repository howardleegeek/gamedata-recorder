...
def _run_tests_with_client(client):
    ...
    r = client.post(
        "/api/v1/auth/login",
        json={"email": "test@gamedatalabs.com", "password": "testpassword123"},
    )
    assert r.status_code == 200
    token = r.json()["token"]
    ...
    r = client.post(
        "/api/v1/upload/chunk",
        headers=headers,
        params={"upload_id": upload_id, "chunk_number": 1},
    )
    assert r.status_code == 200
    ...
    r = client.post(
        ...
        headers=headers,
        json={"upload_id": upload_id, "etags": ["abc"]},
    )
    assert r.status_code == 200
    ...
    print(
        f"[PASS] Earnings: total=${r.json()['total_usd']}, hours={r.json()['hours_recorded_total']}"
    )
    ...
    assert r.status_code == 200
    print(
        f"[PASS] Stats: {r.json()['total_uploads']} uploads, {r.json()['total_size_bytes']} bytes"
    )
