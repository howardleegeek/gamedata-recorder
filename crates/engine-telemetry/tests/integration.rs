...
// `0.7071` (≈ sqrt(2)/2) shows up in this file as a deliberate wire-format
// test fixture — it's the conventional human-readable quaternion component
// for a 90° rotation, and the buyer's sample data files cite the same
// literal. Substituting `FRAC_1_SQRT_2` would obscure the wire contract;
// the value is load-bearing exactly as written. Suppress
// `clippy::approx_constant` at the file level so the spec's
// `-D warnings` clippy gate stays clean without changing test assertions.
